...
int FirstPass(char **files, size_t files_size) {
    ...
    for (size_t i = 0; i < files_size; i++) {
        ...
        printf("(+) Entries file successfully written for file: '%s'\n", files[i]);

        if (GetOutputPath(files[i], ent_output, sizeof(ent_output), ".ext") != 0) {
            printf("(-) Error creating .ext file path for '%s'\n", files[i]);
            continue;
        }

        if (FormatExternalFile(ent_output, labels, &label_count) != 0) {
            printf("(-) Error writing .ent file for '%s'\n", files[i]);
        }
